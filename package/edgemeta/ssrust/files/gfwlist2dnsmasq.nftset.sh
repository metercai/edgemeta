...
#                _red 'Error: Please enter a valid IP set name.\n'
                _green 'nftset replace ipset\n'                                           
                NFT_TABLE=$(echo $IPSET_NAME | awk -F# '{print$1}')
                NFT_SET=$(echo $IPSET_NAME | awk -F# '{print$2}')
                WITH_IPSET=2
#                exit 1
            else
                WITH_IPSET=1
...
ipset=/\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
            ...
            if [ $WITH_IPSET -eq 2 ]; then                                                                       
                _green 'Nftset rules included.'                                                                   
                sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#server=/\1/'$DNS_IP'\#'$DNS_PORT'\
nftset=/\1/'$NFT_TABLE'\#'$NFT_SET'#g' > $CONF_TMP_FILE                      
            else                                                                                                 
